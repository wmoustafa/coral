...
public class GremlinTestUtils {
  ...
  public static void setup() throws IOException, HiveException, MetaException {
    ...
    try {
      ...
      driver.run("CREATE TABLE IF NOT EXISTS default.members(" +
          "member STRING, name STRING, age INT, location STRING)");
      driver.run("CREATE TABLE IF NOT EXISTS default.connections(" +
          "src_member STRING, dst_member STRING)");
    } catch (Exception e) {
    ...
    // Create converter with explicit column names - RelBuilder will have access to tables via HiveSchema
    converter = new GremlinToRelConverter(mscAdapter, VERTEX_TABLE, EDGE_TABLE, 
